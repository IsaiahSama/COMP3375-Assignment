...
router = APIRouter(
    prefix="/htmx", tags=["HTMX"]
)  # This router will be specific for serving data to HTMX requesting services.
...
async def report_image_upload(request: Request):
    ...
    return templates.TemplateResponse(
        "components/image_upload.html", context={"request": request}
    )
...
async def report_location_upload(request: Request):
    ...
    return templates.TemplateResponse(
        "components/location_upload.html", context={"request": request}
    )
...
async def report_failure(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        context={
            "request": request,
            "result": "Failed!",
            "message": "Your report could not be uploaded. Try again later",
        },
    )
